...
class Users(db.Model):
    ...
    is_female = db.Column(db.Boolean, default=True,)  # 남성/여성을 bool로 표현
    ...
    def get_data_object(self, need_feeds=False):
        data = {
            ...
            'profile_img_url': f"https://pythonproject202201khh.s3.ap-northeast-2.amazonaws.com/{self.profile_img_url}" if self.profile_img_url else None, # 프사가 있다면, S3주소로 가공해서. 없다면 None.
    ...
    # 함수 추가 - 비밀번호 원문을 받아서 => 암호화 해주는 함수 생성
    def generate_password_hash(self, input_password):
        
        # 입력받은 비번을 SHA512로 변환해보자 (맞추기 어렵다 / 길이가 길게 나옴) 1차 암호화
        pre_hashed = hashlib.sha512( input_password.encode('utf8') ).hexdigest()
        
        # 1차 암호화된 결과물을 다시 md5로 변환해보자 hashlib 모듈 활용 2차 암호화
        return hashlib.md5( pre_hashed.encode('utf8') ).hexdigest()
    
    # 비번 원문을 받아서 => 비밀번호가 맞는 비번인지 암호화된 값끼리 비교해보는 함수
