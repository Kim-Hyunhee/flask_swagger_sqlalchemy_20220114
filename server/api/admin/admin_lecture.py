...
class AdminLecture (Resource):
    ...
    def post(self):
        ...
        return {
            'code' : 200,
            'message' : '관리자 - 강의 추가 등록 성공',
        }
        
    @swagger.doc({
        'tags' : ['admin'],
        'description' : '관리자 - 강의 과목의 정보 항목 수정',
        'parameters' : [
            {
                'name' : 'X-Http-Token',
                'description' : '사용자 인증용 헤더 - 관리자만 OK',
            ...
            {
                'name' : 'field',
                'description' : '어느 항목을 수정하고 싶은지 - title, campus, fee, teacher_id 중 택 1',
                'in' : 'formData',
                'type' : 'string',
            ...
            {
                'name' : 'value',
                'description' : '어떤 값으로 수정할 것인지 실제 입력값',
                'in' : 'formData',
