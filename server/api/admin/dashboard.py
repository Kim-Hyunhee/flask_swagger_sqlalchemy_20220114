...
class AdminDashboard (Resource):
    ...
    def get(self):
        ...
        # 최근 10일(2022-01-10이후)간의 일자별 매출 총 합
        
        # 지금으로부터 10일 전은 몇 일인가? 구해서 쿼리에 반영하자
        ...
        # 매출이 없는 날은 DB 쿼리 결과도 아예 없다. => 목록에 아예 등록이 안됨
        # 날짜는 무조건 10개, 매출이 없는 날은 0원 처리
        
        # 10일 전 ~ 오늘까지를 for문으로 
        
        for i in range(0, 11):
            
            # 시간 / 분 / 초까지 날짜에 들어감 => 2022- 01 -20 양식으로 변경 
            # 해당 날짜에서 하루 지난 날로 변경
