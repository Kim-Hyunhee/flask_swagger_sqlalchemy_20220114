...
class User(Resource):
    ...
    @swagger.doc({
        ...
        'parameters': [
            ...
            {
                'name' : 'name',
                'description' : '검색해 볼 이름 - 일부분만 일치해도 찾아줌',
                'in' : 'query',
                'type' : 'string',
                'required' : False
            },
    ...
    def get(self):
        ...
        if args['name']:
            # 이메일은 첨부가 안 되어 있어야 함 (이메일 존재 시 위에 if 문에서 끝남)
            
            # ex. "경" => 조경진 / 박진경 등등 여러 경우가 결과로 나올 수 있다. => 검색 결과 여러 개로 all()
    ...
    def delete(self):
        ...
        # db.session.delete(delete_user)  # 누구를 삭제할 지 명시
        # db.session.commit()  # 실제 삭제 수행 => 이 사용자의 활동 내역도 다같이 지워야 정상 동작
        
        
        # 실무 : 기존 데이터를 임시 데이터로 변경
        delete_user.name = '탈퇴한 회원'
    ...
    @swagger.doc({
        ...
        'parameters' : [
            {
                'name' : 'user_id',
                'description' : '몇 번 사용자의 정보를 수정할 지?',
                'in' : 'formData',
                'type' : 'integer',
                'required' : True
            },
