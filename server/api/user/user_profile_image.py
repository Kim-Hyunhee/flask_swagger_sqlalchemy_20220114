...
class UserProfileImage(Resource):
    ...
    def put(self) :
        ...
        for file in args['profile_image']:
            ...
            # 파일 이름 / 확장자 중 확장자만 변수에 담자
            _, file_extension = os.path.splitext(file.filename)  # 원래 올라온 파일명을 => 파일이름/확장자로 분리
            
            new_file_name = f"{new_file_name}{file_extension}"  # MYSNS_123456asddf_12345.png 등 파일명 완성
            
            # 최종 경로 => 1,2의 합체 + S3의 폴더 지정
            s3_file_path = f'images/profile_imgs/{new_file_name}'   # 올라갈 경로 생성 => S3 내부 최종 경로
